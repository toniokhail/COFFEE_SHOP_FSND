...
def get_token_auth_header():
    ...
    authent_header  = request.headers.get('Authorization', None)
    headers_parts = authent_header.split(' ')
    len_headers = len(headers_parts)
    parts_0 = headers_parts[0].lower()
    token = headers_parts[1]

    if not authent_header:
        raise AuthError({
